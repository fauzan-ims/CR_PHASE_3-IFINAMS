...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './salebiddinghistorydetail.component.html'
})

export class SaleBiddingHistoryDetailComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    public SaleBiddingHistoryDetailData: any    = [];
    public lookupAsset: any              = [];
    public isReadOnly: Boolean           = false;
    private dataTamp: any                = [];

    //controller
    private APIController: String        = 'SaleBiddingDetailHistory';
    ...
    //#region form submit
    onFormSubmit(SaleBiddingDetailHistoryForm: NgForm, isValid: boolean) {
        ...
        this.SaleBiddingHistoryDetailData = this.JSToNumberFloats(SaleBiddingDetailHistoryForm);

        const usersJson: any[] = Array.of(this.SaleBiddingHistoryDetailData);
        if (this.paramss != null) {
            // call web service
                ...
                .subscribe(
                    res => {
                        ...
                        if (parse.result === 1) {
                            $('#clientDetail', parent.document).click();
                            this.showNotification('top', 'right', 'success');
                            this.route.navigate(['/inquiry/subsalehistorylist/salehistorydetail/' + this.param + '/salebiddinghistorylist/' + this.param + '/salebiddingdetailhistorydetail', this.param, this.params, parse.id], { skipLocationChange: true });
                        } else {
                            this.swalPopUpMsg(parse.data);
                        }
                    },
    ...
    //#region button back
    btnBack() {
        this.route.navigate(['/inquiry/subsalehistorylist/salehistorydetail/' + this.param + '/salebiddinghistorylist/' + this.param + '/salebiddinghistorydetail', this.param, this.params], { skipLocationChange: true });
