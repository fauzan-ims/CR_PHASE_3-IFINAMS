...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './maintenancereturnapproval.component.html'
})

export class MaintenanceReturnApprovalComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    public isFreeService: Boolean = false;
    public freeservice: Boolean = false;
    public isRequestReplacement: Boolean = false;
    public reqReplacement: Boolean = false;
    // public freeservice: String;
    public lookupapproval: any = [];
    public isDateApprove: Boolean = false;
    public tampHidden: Boolean = false;
    public tampHiddenforView: Boolean = false;
    public listmaintenancedetailBefore: any = [];

    //controller
    private APIController: String = 'Maintenance';
    ...
    private APIControllerSysGeneralCode: String = 'SysGeneralSubcode';
    private APIControllerApprovalSchedule: String = 'ApprovalSchedule';
    private APIControllerReturn: String = 'MaintenanceHistory';
    private APIControllerReturnDetail: String = 'MaintenanceDetailHistory';
    ...
    private APIRouteForUpdateforLookup: String = 'UpdateForLookup';
    ...
    private APIRouteForCancelAfterPost: String = 'ExecSpForCancelAfterPost';
    ...
    private RoleAccessCode = 'R00021790000000A';
    ...
    dtOptionsDetailBefore: DataTables.Settings = {};
    ...
    ngOnInit() {
        ...
        if (this.param != null) {
            ...
            this.maintenancetype = 'INT';
            this.model.transaction_amount = 0;
            this.model.service_type = 'ROUTINE';
            this.isFreeService = false;
            this.isRequestReplacement = false;
        }
    }
            ...
            .subscribe(
                res => {
                    ...
                    if (parsedata.status !== 'APPROVE') {
                        this.isDateApprove = true;
                    } else {
                        this.isDateApprove = false;
                    }

                    if (parsedata.status === 'ON PROCESS' || parsedata.status === 'POST') {
                    ...
                    if (parsedata.free_service === '1') {
                        parsedata.free_service = true;
                        this.isFreeService = true;
                    } else {
                        parsedata.free_service = false;
                        this.isFreeService = false;
                    }

                    if (parsedata.is_request_replacement === '1') {
                        parsedata.is_request_replacement = true;
                        this.isRequestReplacement = true;
                    } else {
                        parsedata.is_request_replacement = false;
                        this.isRequestReplacement = false;
                    }
                    // end checkbox
    ...
    callGetrowBefore() {
        ...
        this.dalservice.Getrow(this.dataTamp, this.APIControllerReturn, this.APIRouteForGetRow)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
                    const parsedata = this.getrowNgb(parse.data[0]);
                    ...
                    // mapper dbtoui
                    Object.assign(this.modelBefore, parsedata);
                    // end mapper dbtoui
    ...
    onFormSubmit(MaintenanceForm: NgForm, isValid: boolean) {
        ...
        if (this.maintenanceData.p_is_reimburse == null || this.maintenanceData.p_is_reimburse == '') {
            this.maintenanceData.p_is_reimburse = false;
        }
        if (this.maintenanceData.p_is_request_replacement == null || this.maintenanceData.p_is_request_replacement == '') {
            this.maintenanceData.p_is_request_replacement = false;
        }

        // if (this.maintenanceData.p_is_reimburse === false) {
        //     this.maintenanceData.p_is_reimburse = '0'
        //   } else {
        //     this.maintenanceData.p_is_reimburse = '1'
        //   }
    ...
    //#region button back
    btnBack() {
        this.route.navigate(['/transaction/submaintenance']);
    ...
    PageStatus(event: any) {
        this.maintenancetype = event.target.value;
        this.model.vendor_name = undefined;
        this.model.vendor_code = undefined;
        // this.model.vendor_bank_name = undefined;
        this.model.vendor_bank_account_no = undefined;
        this.model.vendor_bank_account_name = undefined;
        this.model.bank_code = undefined;
        this.model.sa_vendor_name = undefined;
        this.model.sa_vendor_area_phone = undefined;
        this.model.sa_vendor_phone_no = undefined;
        $('#datatable').DataTable().ajax.reload();
    }
    //#endregion ddl master module

    //#region ddl master module
    FreeService(event: any) {
        this.freeservice = event.target.checked;
        if (this.freeservice == true) {
            this.isFreeService = true;
        } else {
            this.isFreeService = false;
            this.model.bank_account_no = '';
            this.model.bank_account_name = '';
            this.model.vendor_name = '';
            this.model.bank_code = '';
            this.model.bank_name = '';
            this.model.vendor_code = '';
            this.model.vendor_bank_name = '';
            this.model.vendor_bank_account_no = '';
            this.model.vendor_bank_account_name = '';
            this.model.vendor_npwp = '';
            this.model.vendor_type = '';
            this.model.vendor_address = '';
            this.model.sa_vendor_name = '';
            this.model.sa_vendor_area_phone = '';
            this.model.sa_vendor_phone_no = '';
        }

        $('#datatable').DataTable().ajax.reload();
    }

    RequestReplacement(event: any) {
        this.reqReplacement = event.target.checked;
        if (this.reqReplacement == true) {
            this.isRequestReplacement = true;
        } else {
            this.isRequestReplacement = false;
            this.model.delivery_address = '';
            this.model.contact_name = '';
            this.model.contact_phone_no = '';
    ...
    btnCancelAfterPost() {
        ...
        swal({
            ...
            if (result.value) {
                this.dalservice.ExecSp(this.dataRoleTamp, this.APIController, this.APIRouteForCancelAfterPost)
                    .subscribe(
                        res => {
                            this.showSpinner = false;
                            const parse = JSON.parse(res);
    ...
    btnSelectRowAsset(code: String, item_name: String, item_code: String, division_code: String, division_name: String,
        department_code: String, department_name: String, type_code: String, plat_no: String, is_use_maintenance: String) {
        ...
        this.model.is_use_maintenance = is_use_maintenance;
    ...
    btnSelectRowAssetDetail(code: String, description: String, service_type: string) {
        ...
        this.dalservice.Update(this.listdataDetail, this.APIControllerDetail, this.APIRouteForUpdateforLookup)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
                    if (parse.result === 1) {
    ...
    //#region loadDataBefore
    loadDataBefore() {
        this.dtOptionsDetailBefore = {
            pagingType: 'full_numbers',
            responsive: true,
            serverSide: true,
            processing: true,
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIControllerReturnDetail, this.APIRouteForGetRows).subscribe(resp => {
                    const parse = JSON.parse(resp);

                    for (let i = 0; i < parse.data.length; i++) {
                        this.listmaintenancedetailBefore = parse.data;
                    }

                    this.listmaintenancedetailBefore = parse.data;

                    if (parse.data != null) {
                        this.listmaintenancedetailBefore.numberIndex = dtParameters.start;
    ...
    btnSaveList() {
        ...
        const getServiceCode = $('[name="p_service_code"]')
            .map(function () { return $(this).val(); }).get();

        const getQty = $('[name="p_quantity"]')
            .map(function () { return $(this).val(); }).get();

        const getServiceName = $('[name="p_service_name"]')
            .map(function () { return $(this).val(); }).get();

        const getServiceType = $('[name="p_service_type"]')
            .map(function () { return $(this).val(); }).get();

        while (j < getID.length) {

            while (j < getServiceCode.length) {

                while (j < getServiceName.length) {

                    while (j < getServiceType.length) {
                        while (j < getQty.length) {
                            if (getServiceCode[j] == null) {
                                swal({
                                    title: 'Warning',
                            ...
                            this.datauploadlist.push(
                                {
                                    p_id: getID[j],
                                    p_asset_code: getServiceCode[j],
                                    p_quantity: getQty[j],
                                    p_service_type: getServiceType[j],
                                    p_service_name: getServiceName[j],
                                });
                            j++;
                        }
                        j++;
    ...
    btnSelectRowVendor(code: string, name: string, bank_name: string, bank_account_no: string, bank_account_name: string, npwp: string, vendor_type: string, address: string) {
        this.model.vendor_code = code;
        this.model.vendor_name = name;
        this.model.vendor_bank_name = bank_name;
        this.model.vendor_bank_account_no = bank_account_no;
        this.model.vendor_bank_account_name = bank_account_name;
        this.model.vendor_npwp = npwp;
        this.model.vendor_type = vendor_type;
        this.model.vendor_address = address;
        // if (this.model.maintenance_by === 'CST') {
        //     this.model.vendor_bank_name = undefined;
        //     this.model.vendor_bank_account_no = undefined;
        //     this.model.vendor_bank_account_name = undefined;
        //     this.model.vendor_npwp = undefined;
        //     this.model.vendor_type = undefined;
        // }
        // else {
        //     this.model.vendor_bank_name = bank_name;
        //     this.model.vendor_bank_account_no = bank_account_no;
    ...
    btnPrint() {
        ...
        const dataParam = {
            TableName: 'RPT_SURAT_PERINTAH_KERJA',
            SpName: 'xsp_rpt_surat_perintah_kerja',
            reportparameters: {
    ...
    btnLookupReason() {
        ...
        $('#datatableLookupReason').DataTable({
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                // end param tambahan untuk getrows dynamic
                this.dalservice.Getrows(dtParameters, this.APIControllerSysGeneralCode, this.APIRouteLookup).subscribe(resp => {
                    const parse = JSON.parse(resp);
    ...
    btnSelectRowReason(code: string, general_subcode_desc: string) {
        ...
        this.model.reason_name = general_subcode_desc;
    ...
    btnReturn(MaintenanceForm: NgForm, isValid: boolean) {
        ...
        // this.showSpinner = true;
        // }
        this.maintenanceData = MaintenanceForm;

        // param tambahan untuk button Proceed dynamic
        this.dataRoleTamp = [{
            'p_code': this.param,
            'p_remark_return': this.maintenanceData.p_remark_return,
            'action': ''
        }];
        // param tambahan untuk button Proceed dynamic
    ...
    //#region button Done
    btnPost(MaintenanceForm: NgForm, isValid: boolean) {
        ...
        this.dataRoleTamp = [{
            'p_code': this.param,
            'p_start_date': this.formatDateFromPicker(this.maintenanceData.p_start_date),
            'p_finish_date': this.formatDateFromPicker(this.maintenanceData.p_finish_date),
            'action': ''
        }];
