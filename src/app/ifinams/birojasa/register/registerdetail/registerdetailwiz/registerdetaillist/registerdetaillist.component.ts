...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './registerdetaillist.component.html'
})

export class RegisterdetaillistComponent extends BaseComponent implements OnInit {
    // get param from url
    param = this.getRouteparam.snapshot.paramMap.get('id');
    status = this.getRouteparam.snapshot.paramMap.get('status');

    // variable
    public listregisterdetail: any = [];
    ...
    // API Controller
    private APIController: String = 'RegisterDetail';
    private APIControllerRegisterMain: String = 'RegisterMain';
    private APIControllerGeneralSubCode: String = 'SysGeneralSubCode';
    private APIRouteForGetRowsForSysGeneralSubCode: String = 'GetRowsForLookupForRegister';
    ...
    //#region registerdetailList load all data
    loadData() {
        this.dtOptions = {
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    this.listregisterdetail = parse.data;
                    if (parse.data != null) {
                        this.listregisterdetail.numberIndex = dtParameters.start;
                    }
    ...
    btnDeleteAll() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listregisterdetail.length; i++) {
            if (this.listregisterdetail[i].selected) {
                this.checkedList.push(this.listregisterdetail[i].id);
            }
        }
    ...
    selectAllTable() {
        for (let i = 0; i < this.listregisterdetail.length; i++) {
            this.listregisterdetail[i].selected = this.selectedAll;
        }
    }

    checkIfAllTableSelected() {
        this.selectedAll = this.listregisterdetail.every(function (item: any) {
    ...
    btnSelectAllLookup() {
        this.isBreak = false;
        this.checkedLookup = [];
        for (let i = 0; i < this.lookupgeneralsubcode.length; i++) {
            if (this.lookupgeneralsubcode[i].selectedLookup) {
                this.checkedLookup.push(this.lookupgeneralsubcode[i].code);
            }
        }
                    ...
                    res => {
                        ...
                        if (parse.result === 1) {
                            if (J + 1 === this.checkedLookup.length) {
                                ...
                                this.showNotification('bottom', 'right', 'success');
                                $('#datatabless').DataTable().ajax.reload(null, false);
                                $('#datatableLookupGeneralSubCode').DataTable().ajax.reload(null, false);
                            }
                        } else {
                            this.isBreak = true;
                            this.showSpinner = false;
    ...
    selectAllLookup() {
        for (let i = 0; i < this.lookupgeneralsubcode.length; i++) {
            this.lookupgeneralsubcode[i].selectedLookup = this.selectedAllLookup;
        }
    }

    checkIfAllLookupSelected() {
        this.selectedAllLookup = this.lookupgeneralsubcode.every(function (item: any) {
    ...
    btnLookupGeneralSubCode() {
        ...
        $('#datatableLookupGeneralSubCode').DataTable({
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIControllerGeneralSubCode, this.APIRouteForGetRowsForSysGeneralSubCode).subscribe(resp => {
                    // if use checkAll use this
