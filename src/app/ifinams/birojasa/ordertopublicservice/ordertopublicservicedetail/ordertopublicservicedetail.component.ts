...
export class OrdertopublicservicedetailComponent extends BaseComponent implements OnInit {
        ...
        for (let i = 0; i < this.listorderdetail.length; i++) {
            if (this.listorderdetail[i].selected) {
                this.checkedList.push({
                    'Id': this.listorderdetail[i].id,
                    'Code': this.listorderdetail[i].code,
                })
            }
        }
                        ...
                        (function loopProcurementProceed() {
                            if (i < th.checkedList.length) {
                                th.dataTampProceed = [{
                                    ...
                                    'p_order_code': th.checkedList[i].Code
                                }];
                                //Proceed data dan insert into quotation / supplier selection
                                th.dalservice.ExecSp(th.dataTampProceed, th.APIControllerOrderDetail, th.APIRouteForGetDelete)
                                    .subscribe(
                                        res => {
                                            const parse = JSON.parse(res);
                                            if (parse.result === 1) {
    ...
    btnBack() {
        ...
        this.route.navigate(['/birojasa/ordertobirojasalist/']);
    }
        ...
        $('#datatableLookupRegisterMain').DataTable({
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIControllerRegisterMain, this.APIRouteLookupRegisterMain).subscribe(resp => {
                    ...
                    if (parse.data != null) {
                        this.lookupregistermain.numberIndex = dtParameters.start;
                    }
                    // if use checkAll use this
    ...
    btnPrintSuratKuasa() {
        this.showSpinner = true;
        const dataParam = {
            TableName: 'RPT_SURAT_KUASA',
            SpName: 'xsp_rpt_surat_kuasa',
            reportparameters: {
                p_user_id: this.userId,
                p_code: this.param,
                p_order_no: this.model.code,
                p_print_option: 'PDF'
            }
        };
