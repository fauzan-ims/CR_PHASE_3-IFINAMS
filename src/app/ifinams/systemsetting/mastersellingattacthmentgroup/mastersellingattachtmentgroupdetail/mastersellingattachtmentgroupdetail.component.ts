...
@Component({
    selector: 'app-root',
    templateUrl: './mastersellingattachtmentgroupdetail.component.html'
})
export class MasterSellingAttachtmentGroupdetailComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    //controller
    private APIController: String = 'MasterSellingAttachmentGroup';
    ...
    private APIControllerDetail: String = 'MasterSellingAttachtmentGroupDetail';
    ...
    private RoleAccessCode = 'R00024700000001A'; // role access 
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIControllerDetail, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    for (let i = 0; i < parse.data.length; i++) {
                        // checkbox
                        if (parse.data[i].is_required === '1') {
                            parse.data[i].is_required = true;
                        } else {
                            parse.data[i].is_required = false;
                        }
                        // end checkbox
    ...
    //#region sellType
    sellType(event) {
        this.model.sell_type = event.target.value;
    }
    //#endregion sellType
                ...
                .subscribe(
                    res => {
                        ...
                        if (parse.result === 1) {
                            this.showSpinner = false;
                            this.showNotification('bottom', 'right', 'success');
                            this.route.navigate(['/systemsetting/submastersellingattachmentgroup/mastersellingattachtmentgroupdetail', parse.code]);
                        } else {
                            this.showSpinner = false;
                            this.swalPopUpMsg(parse.data)
                        }
    ...
    //#region button back
    btnBack() {
        this.route.navigate(['/systemsetting/submastersellingattachmentgroup']);
    ...
    btnSelectRowType(code: string, general_subcode_desc: string) {
        this.model.document_group_type_code = code;
    ...
    //#region checkIfSelected
    checkIfSelected(isRequired, id) {
        this.id = id;
        this.isRequired = isRequired.target.checked;
        this.tamps.push({
            p_id: this.id,
            p_is_required: this.isRequired
        });

        //#region web service
        this.dalservice.Update(this.tamps, this.APIControllerDetail, this.APIRouteForUpdate)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
                    if (parse.result === 1) {
                        ...
                        $('#datatableDocGroupDetail').DataTable().ajax.reload();
                    } else {
                        this.tamps = new Array();
                        this.swalPopUpMsg(parse.data)
                        $('#datatableDocGroupDetail').DataTable().ajax.reload();
