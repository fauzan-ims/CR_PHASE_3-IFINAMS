...
export class GllinklistComponent extends BaseComponent implements OnInit {
  ...
  private RoleAccessCode = 'R00021310000000A';
  ...
  loadData() {
    this.dtOptions = {
      ...
      columnDefs: [{ orderable: false, searchable: false, width: '5%', targets: [0, 1, 6] }], // for disabled coloumn
      language: {
        search: '_INPUT_',
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listgllink.length; i++) {
      if (this.listgllink[i].selected) {
        this.checkedList.push(this.listgllink[i].code);
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listgllink.length; i++) {
      this.listgllink[i].selected = this.selectedAll;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAll = this.listgllink.every(function (item: any) {
