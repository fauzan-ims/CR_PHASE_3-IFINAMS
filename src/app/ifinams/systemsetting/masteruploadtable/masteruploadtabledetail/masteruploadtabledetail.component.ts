...
export class MasterUploadTabledetailComponent extends BaseComponent implements OnInit {
  ...
  onFormSubmit(masteruploadtabledetailForm: NgForm, isValid: boolean) {
    ...
    if (this.masteruploadtabledetailData.p_is_active == null) {
      this.masteruploadtabledetailData.p_is_active = true;
    }
      ...
      this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/systemsetting/masteruploadtabledetail', parse.code]);
            } else {
              this.swalPopUpMsg(parse.data);
            }
          },
  ...
  btnBack() {
    ...
    this.route.navigate(['/systemsetting/submasteruploadtablelist']);
  }
  ...
  btnEdit(codeEdit: string) {
    this.route.navigate(['/systemsetting/submasteruploadtablelist/masteruploadtabledetaildetail', this.param, codeEdit, this.model.table_name]);
  }
  ...
  btnSelectAllLookup() {
    ...
    for (let i = 0; i < this.lookupuploadtablecolumn.length; i++) {
      if (this.lookupuploadtablecolumn[i].selectedLookup) {
        this.checkedLookup.push({
          'column_name': this.lookupuploadtablecolumn[i].column_name
          , 'data_type': this.lookupuploadtablecolumn[i].data_type
          , 'max_length': this.lookupuploadtablecolumn[i].max_length
        });
      }
    }
    ...
    for (let J = 0; J < this.checkedLookup.length; J++) {
      // param tambahan untuk getrow dynamic
      const columnname = this.checkedLookup[J].column_name;
      const datatype = this.checkedLookup[J].data_type;
      const maxlength = this.checkedLookup[J].max_length;
