...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './reversedocumentdisposalhistorywizlist.component.html'
})

export class ReverseDocumentDisposalHistoryWizListComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    //controller
    private APIController: String = 'ReverseDisposalDocumentHistory';
    private APIControllerOrder: String = 'ReverseDisposalHistory';
        ...
        this.dalservice.Insert(this.dataRoleTamp, this.APIController, this.APIRouteForInsert)
            .subscribe(
                res => {
                    ...
                    if (parse.result === 1) {
                        this.route.navigate(['/transaction/subreversedisposal/reversedisposaldetail' + parse.id + '/reversedisposaldocumentlist/' + parse.id + '/reversedisposaldocumentlist', parse.id]);
    ...
    handleFile(event) {
        ...
        this.tamps.push({
            p_module: 'EFAM',
            p_header: 'REVERSE_DISPOSAL_DOCUMENT',
            p_child: this.param,
            p_id: this.tampDocumentCode,
