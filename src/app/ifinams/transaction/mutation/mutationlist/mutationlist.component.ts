...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './mutationlist.component.html'
})

export class MutationlListComponent extends BaseComponent implements OnInit {
    // variable
    public listmutation: any = [];
    public dataTamp: any = [];
    public rptParam: any = [];
    public dataParam: any = [];
    public status: any;
    public choose_file: any;
    ...
    private APIControllerUpload: String = 'MutationUpload';
        ...
        this.dtOptions = {
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    // end checkall
                    this.listmutation = parse.data;

                    if (parse.data != null) {
                        this.listmutation.numberIndex = dtParameters.start;
                    }
    ...
    //#region button add
    btnAdd() {
        this.route.navigate(['/transaction/submutation/mutationdetail']);
    }
    //#endregion button add

    //#region button edit
    btnEdit(codeEdit: string) {
        this.route.navigate(['/transaction/submutation/mutationdetail', codeEdit]);
    }
    //#endregion button edit

    //#region ddl PageChooseFile
    PageChooseFile(event: any) {
        this.choose_file = event.target.value;
        $('#datatable').DataTable().ajax.reload();
    }
    //#endregion ddl PageChooseFile    
    ...
    onUploadReader(event) {
        ...
        if (this.tempFile !== 'MUTATIONUPLOAD.XLSX' && this.tempFile !== 'mutation_upload.xlsx') {
            this.tempFile = '';
            swal({
                title: 'Warning',
    ...
    btnPrintBast() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listmutation.length; i++) {
            if (this.listmutation[i].selected) {
                this.checkedList.push(this.listmutation[i].code);
            }
        }
    ...
    selectAllTable() {
        for (let i = 0; i < this.listmutation.length; i++) {
            this.listmutation[i].selected = this.selectedAll;
        }
    }

    checkIfAllTableSelected() {
        this.selectedAll = this.listmutation.every(function (item: any) {
            return item.selected === true;
        })
    }
    //#endregion btnPrintBast
    ...
    //#region button Print Clearance Resign
    btnPrintClearanceResign() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listmutation.length; i++) {
            if (this.listmutation[i].selected) {
        ...
        swal({
            ...
            if (result.value) {
                ...
                for (let J = 0; J < this.checkedList.length; J++) {
                    ...
                    this.rptParam = {
                        p_user_id: this.userId,
                        p_mutasi_no: this.checkedList[J],
                        p_print_option: 'PDF'
                    }

                    this.dataParam = {
                        TableName: 'RPT_CLEARANCE_SHEET_RESIGN',
                        SpName: 'xsp_rpt_clearance_sheet_resign',
                        reportparameters: this.rptParam
                    };
    ...
    //#endregion button Print Clearance Resign

    //#region button Print Clearance Resign
    btnPrintClearanceResigns() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listmutation.length; i++) {
            if (this.listmutation[i].selected) {
    ...
    //#region button Print Clearance Mutasi
    btnPrintClearanceMutasi() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listmutation.length; i++) {
            if (this.listmutation[i].selected) {
        ...
        swal({
            ...
            if (result.value) {
                ...
                for (let J = 0; J < this.checkedList.length; J++) {
                    ...
                    this.dataParam = {
                        TableName: 'RPT_POLOSAN_CLEARENSHEET_MUTASI',
                        SpName: 'xsp_rpt_polosan_clearensheet_mutasi',
                        reportparameters: this.rptParam
                    };
    ...
    //#endregion button Print Clearance Mutasi

    //#region button Print Clearance Mutasi
    btnPrintClearanceMutasis() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listmutation.length; i++) {
            if (this.listmutation[i].selected) {
