...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './handoverchecklistwizlist.component.html'
})

export class HandoverchecklistwizlistComponent extends BaseComponent implements OnInit {

    param = this.getRouteparam.snapshot.paramMap.get('id');

    // variable
    public listassetchecklist: any = [];
    public listassetchecklistdata: any = [];
    public dataTamp: any = [];
    ...
    //controller
    private APIController: String = 'HandoverAssetChecklist';
    private APIControllerHeader: String = 'HandoverAsset';
        ...
        this.dtOptions = {
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    this.listassetchecklist = parse.data;
                    if (parse.data != null) {
                        this.listassetchecklist.numberIndex = dtParameters.start;
                    }
    ...
    btnDeleteAll() {
        this.checkedList = [];
        for (let i = 0; i < this.listassetchecklist.length; i++) {
            if (this.listassetchecklist[i].selected) {
                this.checkedList.push(this.listassetchecklist[i].id);
            }
        }
    ...
    selectAllTable() {
        for (let i = 0; i < this.listassetchecklist.length; i++) {
            this.listassetchecklist[i].selected = this.selectedAll;
        }
    }

    checkIfAllTableSelected() {
        this.selectedAll = this.listassetchecklist.every(function (item: any) {
    ...
    btnSaveList() {
        ...
        while (i < getID.length) {
                this.listassetchecklistdata.push(
        ...
        //#region web service
        this.dalservice.Update(this.listassetchecklistdata, this.APIController, this.APIRouteForUpdate)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
