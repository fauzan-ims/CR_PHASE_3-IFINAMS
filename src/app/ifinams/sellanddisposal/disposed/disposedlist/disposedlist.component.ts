...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './disposedlist.component.html'
})

export class DisposedListComponent extends BaseComponent implements OnInit {
    // variable
    public listdisposal: any = [];
    public dataTamp: any = [];
        ...
        this.dtOptions = {
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    // end checkall
                    this.listdisposal = parse.data;

                    if (parse.data != null) {
                        this.listdisposal.numberIndex = dtParameters.start;
                    }
    ...
    //#region button add
    btnAdd() {
        this.route.navigate(['/sellanddisposal/subdisposedlist/disposaldetail']);
    }
    //#endregion button add

    //#region button edit
    btnEdit(codeEdit: string) {
        this.route.navigate(['/sellanddisposal/subdisposedlist/disposaldetail', codeEdit]);
    }
    ...
    btnPrint() {
        this.isBreak = false;
        this.checkedList = [];
        for (let i = 0; i < this.listdisposal.length; i++) {
            if (this.listdisposal[i].selected) {
                this.checkedList.push(this.listdisposal[i].code);
            }
        }
    ...
    selectAllTable() {
        for (let i = 0; i < this.listdisposal.length; i++) {
            this.listdisposal[i].selected = this.selectedAll;
        }
    }

    checkIfAllTableSelected() {
        this.selectedAll = this.listdisposal.every(function (item: any) {
