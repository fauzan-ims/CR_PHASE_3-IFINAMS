...
@Component({
  selector: 'app-reportprofitlossasset',
  templateUrl: './reportprofitlossasset.component.html'
})
export class Reportprofitlossasset extends BaseComponent implements OnInit {
  // get param from url
  ...
  private APIRouteForGetrowsVendor: String = 'GetRowsForLookupReportVendor';
  ...
  //lookup Locker
  private APIControllerLocker = 'DocumentMain';
  private APIRouteForLookupLocker = 'GetRowsForLookupReportLocker';
  ...
  btnLookupVendor() {
    ...
    $('#datatableLookupVendor').DataTable({
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        // end param tambahan untuk getrows dynamic
        this.dalservice.GetrowsBam(dtParameters, this.APIControllerVendor, this.APIRouteForGetrowsVendor).subscribe(resp => {
  ...
  //#region Locker Lookup
  btnLookupLocker() {
    $('#datatableLookupLocker').DataTable().clear().destroy();
    $('#datatableLookupLocker').DataTable({
      'pagingType': 'first_last_numbers',
      'pageLength': 5,
      'processing': true,
      'serverSide': true,
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        // end param tambahan untuk getrows dynamic
        this.dalservice.Getrows(dtParameters, this.APIControllerLocker, this.APIRouteForLookupLocker).subscribe(resp => {
          const parse = JSON.parse(resp);
          this.lookuplocker = parse.data;
          if (parse.data != null) {
            this.lookuplocker.numberIndex = dtParameters.start;
          }
          callback({
            draw: parse.draw,
            recordsTotal: parse.recordsTotal,
  ...
  btnSelectRowlookupLocker(code: String, description: String) {
    this.model.locker_code = code;
    this.model.locker_name = description;
    $('#lookupModalLocker').modal('hide');
  }
  //#endregion Locker lookup
