...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './reversedisposaldetail.component.html'
})

export class ReverseDisposaldetailComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    private dataTampReverseDisposalData: any = [];
    public dataRoleTampInsertLog: any = [];
    public jsonDataJournal: any = [];

    //controller
    private APIController: String = 'ReverseDisposal';
    ...
    private RoleAccessCode = 'R00021680000000A';
    ...
    disposaldetailwiz() {
        this.route.navigate(['/transaction/subreversedisposal/reversedisposaldetail/' + this.param + '/reversedisposaldetaillist', this.param], { skipLocationChange: true });
    }

    disposaldocumentwiz() {
        this.route.navigate(['/transaction/subreversedisposal/reversedisposaldetail/' + this.param + '/reversedisposaldocumentlist/' + this.param + '/' + this.disposal_code + '/disposaldocumentlist', this.param, this.disposal_code], { skipLocationChange: true });
    }
    ...
    //#region button back
    btnBack() {
        this.route.navigate(['/transaction/subreversedisposal']);
    ...
    //#region btnPost
    btnPost() {
        this.dataTampReverseDisposalData = [{
            'p_code': this.param,
            'action': 'default'
        }];
        ...
        swal({
            ...
            this.showSpinner = true;
            if (result.value) {
                this.dalservice.ExecSp(this.dataTampReverseDisposalData, this.APIController, this.APIRouteForPost)
                    .subscribe(
                        resDataReverseDisposal => {
                            const parseDataReverseDisposal = JSON.parse(resDataReverseDisposal);

                            if (parseDataReverseDisposal.result === 1) {
                                this.dataTamp = [{
                                    'p_code': "AJT"
                                }];
                                ...
                                this.swalPopUpMsg(parseDataReverseDisposal.data);
                                this.showSpinner = false;
                            }
                        },
                        error => {
    ...
    returnJournal() {
        ...
        this.dalservice.ExecSp(this.dataTampReverseDisposalData, this.APIController, this.APIRouteForReturnJournal)
            .subscribe(
                resReturn => {
