...
@Component({
  selector: 'app-reportassetactivityprepaid',
  templateUrl: './reportassetactivityprepaid.component.html'
})
export class Reportassetactivityprepaid extends BaseComponent implements OnInit {
  // get param from url
  ...
  //#region onFormSubmit
  onFormSubmit(reportForm: NgForm, isValid: boolean, type: any, print_option: any) {
    if (!isValid) {
      swal({
    ...
    if (type === '1') {
      is_report_name = 'rpt_pa_with_amortization';
    }
    else {
      is_report_name = 'rpt_fa_in_use_book';
    }

    this.reportData = this.JSToNumberFloats(reportForm);
    // this.model.as_of_date = new Date();
    //this.model.as_of_date.setMonth(this.model.as_of_date.getMonth() - 1);

    this.reportData.p_user_id = this.userId;
    this.reportData.p_report_name = is_report_name;
    this.reportData.p_branch_code = this.model.branch_code;
    this.reportData.as_of_date = this.model.as_of_date;
    this.reportData.p_report_name = this.model.name;
    this.reportData.p_is_condition = is_condition;

    if (type == 0) {
      let tablename = "RPT_FA_IN_USE_BOOK"
      let spname = "xsp_rpt_fa_in_use_book"
      const dataParam = {
        TableName: tablename,
        SpName: spname,
      ...
      let tablename = "RPT_PA_WITH_AMORTIZATION"
      let spname = "xsp_rpt_pa_with_amortization"
