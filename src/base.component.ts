...
export abstract class BaseComponent extends BaseService {
    ...
    protected wizardInWizard() {
        //#region wizard init
        setTimeout(function () {
            $('.card.card-wizard-wiz').addClass('active');
        }, 600);

        $('.card-wizard-wiz').bootstrapWizard({
            'tabClass': 'nav nav-pills',
            'nextSelector': '.btn-next',
            'previousSelector': '.btn-previous',
            ...
            onInit: function (tab: any, navigation: any, index: any) {
                ...
                // check number of tabs and fill the entire row
                let $total = navigation.find('li').length;
                const $wizard = navigation.closest('.card-wizard-wiz');

                const $first_li = navigation.find('li:first-child a').html();
                // const $moving_div = $('<div class="moving-tab-wiz">' + $first_li + '</div>');
                // $('.card-wizard-wiz .wizard-navigation-wiz').append($moving_div);
                ...
                $wizard.find('.moving-tab-wiz').css('width', step_width);
                $('.moving-tab-wiz').css({
                    'transform': 'translate3d(' + move_distance + 'px, ' + vertical_level + 'px, 0)',
                    'transition': 'all 0.5s cubic-bezier(0.29, 1.42, 0.79, 1)'

                });
                $('.moving-tab-wiz').css('transition', 'transform 0s');
            },

            onTabClick: function () {

                const $valid = $('.card-wizard-wiz form').valid();
            ...
            onTabShow: function (tab: any, navigation: any, index: any) {
                ...
                setTimeout(function () {
                    $('.moving-tab-wiz').html(button_text);
                }, 150);
    ...
    protected _listdialogconfforwinner = 'Can only choose one winner.';
    // protected _listdialogconf = 'No data selected, select at least one data.';
    ...
    getrowNgb(dob: any) {
        ...
        Object.keys(dob).forEach(function (key) {
            ...
            // console.log(matches);
            if (matches != false && dob[key].split('-').length == 3) {
    ...
    Delimiter(element: any) {
        ...
        if (numberAmt.length > 0) {
            $('[amount]').on('paste keydown keyup blur focus', function (event: any) {
                ...
                if (event.type === 'keydown') {
                    ...
                    if (event.target.value.match('[A-Za-z]')) {
                        event.target.value = event.target.value.replace(event, 0);
                    }
                    else if (event.target.value.match('[0-9]+(,[0-9]+)')) {
                    ...
                    event = '' + event.target.value;
                    $('[amount = "' + numberVal + '"]').css({ 'color': '#252422' });
        ...
        if (numberRate.length > 0) {
            $('[rate]').on('paste keydown keyup blur focus', function (event: any) {
    ...
    //#region  dateFomatList
    dateFomatList(convertDate: any) {
        ...
        if (loc !== '') {
            ...
            route.events.subscribe((event: any) => {
                ...
                if (event instanceof NavigationStart) {
                    ...
                    const forWizardEvent = event.url;

                    if (urlParentWiz !== forWizardEvent) {
                        element.nativeElement.querySelector('#hideMainPanelWiz').style.display = 'none';
                    } else {
                        element.nativeElement.querySelector('#hideMainPanelWiz').style.display = 'block';
                    }
                    ...
                    // const detailWiz = event.url.split('/')[10];
    ...
    formatDateFromPicker(dateObj: any): string | null {
    if (!dateObj || !dateObj.singleDate || !dateObj.singleDate.date) return null;

    const { year, month, day } = dateObj.singleDate.date;
