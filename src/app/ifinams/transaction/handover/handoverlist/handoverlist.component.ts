...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './handoverlist.component.html'
})

export class HandoverListComponent extends BaseComponent implements OnInit {
    // variable
    public listhandover: any = [];
    public dataTamp: any = [];
        ...
        this.dtOptions = {
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    // end checkall
                    this.listhandover = parse.data;

                    if (parse.data != null) {
                        this.listhandover.numberIndex = dtParameters.start;
                    }
    ...
    //#region button add
    btnAdd() {
        this.route.navigate(['/transaction/subhandoverlist/handoverdetail']);
    }
    //#endregion button add

    //#region button edit
    btnEdit(codeEdit: string) {
        this.route.navigate(['/transaction/subhandoverlist/handoverdetail', codeEdit]);
    }
    ...
    selectAllTable() {
        for (let i = 0; i < this.listhandover.length; i++) {
            this.listhandover[i].selected = this.selectedAll;
        }
    }

    checkIfAllTableSelected() {
        this.selectedAll = this.listhandover.every(function (item: any) {
