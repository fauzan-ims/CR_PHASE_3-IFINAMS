...
@Component({
  selector: 'app-reportcontrolcardmaintenance',
  templateUrl: './reportcontrolcardmaintenance.component.html'
})
export class Reportcontrolcardmaintenance extends BaseComponent implements OnInit {
  // get param from url
  ...
  //#region onFormSubmit
  onFormSubmit(rptForm: NgForm, print_option: any, isValid: boolean) {
    if (!isValid) {
      swal({
  ...
  Print() {
    ...
    const dataParam = {
      ...
      reportparameters: {
        ...
        p_code: this.param,
        p_print_option: this.reportData.p_print_option
      }
  ...
  btnSelectRowAgreement(agreement_no: String, agreement_external_no: String, client_name: string) {
    ...
    this.model.asset_code = undefined;
    this.model.item_name = undefined;
  ...
  btnSelectRowAsset(asset_code: String, item_name: string, plat_no: string) {
