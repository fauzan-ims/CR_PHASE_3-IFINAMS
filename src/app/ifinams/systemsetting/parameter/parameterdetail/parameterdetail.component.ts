...
export class ParameterdetailComponent extends BaseComponent implements OnInit {
  ...
  private RoleAccessCode = 'R00021360000000A'; // role access 
  ...
  onFormSubmit(parameterdetailForm: NgForm, isValid: boolean) {
    ...
    if (this.parameterdetailData.p_is_active == null) {
      this.parameterdetailData.p_is_active = true;
    }
      ...
      this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/systemsetting/subparameterlist/parameterdetail', this.parameterdetailData.p_code]);
            } else {
              this.swalPopUpMsg(parse.data);
            }
          },
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/systemsetting/subparameterlist']);
