...
export class SidebarComponent extends BaseComponent implements OnInit {
    ...
    private APIControllerSysMenuAccessLog: String = 'SysMenuAccessLog';
    ...
    private moduleName: String = 'IFINAMS'
    ...
    constructor(private dalservice: DALService, private spinnerService: Ng4LoadingSpinnerService) {
        super();
    }
                        ...
                        this.dalservice.ExecSpSys(this.dataTampMenu, this.APIController, this.APIRouteSidebarForMenuGetRows)
                            .subscribe(
                                ress => {
                                    ...
                                    this.tamp1[k].childrenitemsonesub = submenuItemsData;
                                });
                    }
    ...
    goToMainMenu() {
        const urlMainMenu = this.AllModUrl('urlMainMenu');//this.decryptLogic(sessionStorage.getItem('bWFmRXJkZEFscnU%3D'));

        window.open(urlMainMenu, '_parent'); // sessionStorage.clear();
    }

    //#region updateLog
    updateLog(subMenuName: any, menuName: any) {
        this.dataTamp = [{
            'p_employee_code': this.userId,
            'p_employee_name': this.userName,
            'p_submenu_name': subMenuName,
            'p_menu_name': menuName,
            'p_process_name': 'ACCESS'
        }];
        // call web service
        this.dalservice.Insert(this.dataTamp, this.APIControllerSysMenuAccessLog, this.APIRouteForInsert)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
                    if (parse.result !== 1) {
