...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './assetdetailelectronicwizdetail.component.html'
})

export class AssetDetailElectronicwizdetailComponent extends BaseComponent implements OnInit {
    // get param from url
            ...
            .subscribe(
                res => {
                    ...
                    if (parse.result === 1) {
                        ...
                        this.route.navigate(['/transaction/subasset/assetwizlistdetail/' + this.param + '/assetdetailelectronicdetail', this.param, parse.id]);
                        this.callGetrow()
                    } else {
    ...
    callGetrowHeader() {
        ...
        this.dalservice.Getrow(this.dataTamp, this.APIControllerHeader, this.APIRouteForGetRow)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
                    const parsedataHeader = this.getrowNgb(parse.data[0]);

                    // mapper dbtoui
                    Object.assign(this.modeldetail, parsedataHeader);
                    // end mapper dbtoui
                    this.showSpinner = false;
                },
                error => {
