...
export class SoldSettlementDetailFeewizlistComponent extends BaseComponent implements OnInit {
    ...
    btnSaveList() {
        ...
        .map(function () { return $(this).val(); }).get(); // (+) Ari 2024-01-08 ket : editable ppn amount
            ...
            while (i < getFeeAmount.length) {
                ...
                this.listsalefeeData.push(
                    this.JSToNumberFloats({
                        ...
                        p_ppn_amount : getPpnAmount[i], // (+) Ari 2024-01-08 ket : editable ppn amount
                    })
                );
    ...
    btnSelectRowTax(code: string, description: string, ppn_pct: string, pph_pct: string) {
        ...
        //#region web service
        this.dalservice.Update(listdataDetailWithQuotaion, this.APIController, this.APIRouteForUpdateTax)
            .subscribe(
                res => {
                    const parse = JSON.parse(res);
                    if (parse.result === 1) {
