...
export class SoldRequestdetailComponent extends BaseComponent implements OnInit {
            ...
            this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
                .subscribe(
                    res => {
                        ...
                        if (parse.result === 1) {
                            this.showNotification('bottom', 'right', 'success');
                            this.route.navigate(['/settlement/subsoldrequestlist/soldrequestdetail', parse.code]);
                            this.showSpinner = false;
                        } else {
                            this.swalPopUpMsg(parse.data);
                            this.showSpinner = false;
    ...
    //#region button back
    btnBack() {
        this.route.navigate(['/settlement/subsoldrequestlist']);
    ...
    saledetailwiz() {
        this.route.navigate(['/settlement/subsoldrequestlist/soldrequestdetail/' + this.param + '/soldrequestassetlist', this.param], { skipLocationChange: true });
    }
