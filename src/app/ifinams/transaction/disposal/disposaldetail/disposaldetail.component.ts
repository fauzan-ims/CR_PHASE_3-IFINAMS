...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './disposaldetail.component.html'
})

export class DisposaldetailComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    //#region button back
    btnBack() {
        this.route.navigate(['/transaction/subdisposal']);
    ...
    disposaldetailwiz() {
        this.route.navigate(['/transaction/subdisposal/disposaldetail/' + this.param + '/disposaldetaillist', this.param], { skipLocationChange: true });
    }
    disposaldocumentwiz() {
        this.route.navigate(['/transaction/subdisposal/disposaldetail/' + this.param + '/disposaldocumentlist', this.param], { skipLocationChange: true });
    }
        ...
        swal({
            ...
            if (result.value) {
                this.dalservice.ExecSp(this.dataTampDisposalData, this.APIController, this.APIRouteForPost)
                    .subscribe(
                        resDataDisposal => {
                            const parseDataDisposal = JSON.parse(resDataDisposal);

                            if (parseDataDisposal.result === 1) {
                                this.dataTamp = [{
                                    'p_code': "AJT"
                                }];
                                ...
                                this.swalPopUpMsg(parseDataDisposal.data);
                                this.showSpinner = false;
                            }
                        },
                        error => {
