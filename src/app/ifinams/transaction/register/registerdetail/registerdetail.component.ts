...
export class RegisterdetailComponent extends BaseComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.param != null) {
            ...
            this.model.register_process_by = 'CUSTOMER'
            ...
            this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
                .subscribe(
                    res => {
                        ...
                        if (parse.result === 1) {
                            this.showNotification('bottom', 'right', 'success')
                            this.route.navigate(['/transaction/subregisterbirojasalist/registerdetail', parse.code]);
                        } else {
                            this.swalPopUpMsg(parse.data)
                        }
                    },
    ...
    btnBack() {
        ...
        this.route.navigate(['/transaction/subregisterbirojasalist']);
    }
    ...
    registerdetailwiz() {
        this.route.navigate(['/transaction/subregisterbirojasalist/registerdetail/' + this.param + '/registerdetaillist', this.param], { skipLocationChange: true });
    }
    //#endregion public Detail List tabs

    //#region public document list tabs
    registerdocumentwiz() {
        this.route.navigate(['/transaction/subregisterbirojasalist/registerdetail/' + this.param + '/registerdocumentlist', this.param], { skipLocationChange: true });
    }
    ...
    btnLookupFACode() {
        ...
        $('#datatableLookupFACode').DataTable({
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                // end param tambahan untuk getrows dynamic
                this.dalservice.Getrows(dtParameters, this.APIControllerFACode, this.APIRouteForLookup).subscribe(resp => {
                    const parse = JSON.parse(resp);
    ...
    btnSelectRowFACode(code: String, description: String) {
