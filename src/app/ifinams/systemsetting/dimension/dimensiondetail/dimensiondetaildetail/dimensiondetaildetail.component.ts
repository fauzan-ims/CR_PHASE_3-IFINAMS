...
export class DimensiondetaildetailComponent extends BaseComponent implements OnInit {
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/systemsetting/subdimensionlist/dimensiondetaildetail', this.param, parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data)
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/systemsetting/subdimensionlist/dimensiondetail', this.param]);
  }
