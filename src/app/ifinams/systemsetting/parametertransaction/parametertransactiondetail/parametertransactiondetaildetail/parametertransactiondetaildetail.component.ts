...
export class ParametertransactiondetaildetailComponent extends BaseComponent implements OnInit {
  ...
  private RoleAccessCode = 'R00021370000000A'; // role access 
  ...
  onFormSubmit(parametertransactionForm: NgForm, isValid: boolean) {
    ...
    this.parametertransactionData.p_company_code = this.company_code;
      ...
      this.dalservice.Insert(usersJson, this.APIController, this.APIRouteForInsert)
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/systemsetting/subparametertransactionlist/parametertransactiondetaildetail', this.param, parse.id]);
            } else {
              this.swalPopUpMsg(parse.data);
            }
          },
  ...
  btnBack() {
    ...
    this.route.navigate(['/systemsetting/subparametertransactionlist/parametertransactiondetail', this.param]);
  }
  ...
  btnLookupGlLink() {
    ...
    $('#datatableLookupGlLink').DataTable({
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        // end param tambahan untuk getrows dynamic
        this.dalservice.ExecSpSys(dtParameters, this.APIControllerJournalGlLink, this.APIRouteForLookup).subscribe(resp => {
          const parse = JSON.parse(resp);
