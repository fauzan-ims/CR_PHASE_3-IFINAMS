...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './claimdoclist.component.html'
})

export class ClaimdoclistComponent extends BaseComponent implements OnInit {
  ...
  //controller
  private APIController: String = 'ClaimDoc';
  ...
  //router
  private APIRouteForLookupForDoc: String = 'GetRowsLookupForClaimDoc';
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          this.listclaimdoc = parse.data;
          this.listclaimdoc.numberIndex = dtParameters.start;
  ...
  btnLookupDoc() {
    ...
    $('#datatableLookupDocument').DataTable({
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerSysGeneralDocument, this.APIRouteForLookupForDoc).subscribe(resp => {
          const parse = JSON.parse(resp);
  ...
  btnSelectAllLookup() {
    ...
    for (let i = 0; i < this.lookupdoc.length; i++) {
      if (this.lookupdoc[i].selectedLookup) {
        this.checkedLookup.push({
          codeData: this.lookupdoc[i].code,
          nameData: this.lookupdoc[i].description
        });
      }
    }
    ...
    for (let J = 0; J < this.checkedLookup.length; J++) {
      this.dataTamp.push({
        ...
        'p_document_name': this.checkedLookup[J].nameData
      });
    }
    ...
    this.dalservice.Insert(this.dataTamp, this.APIController, this.APIRouteForInsert)
      .subscribe(
        res => {
          ...
          if (parse.result === 1) {
            this.showSpinner = false;
            $('#datatableClaimDocList').DataTable().ajax.reload();
  ...
  //   this.isBreak = false;
  //   this.checkedList = [];
  //   for (let i = 0; i < this.listclaimdoc.length; i++) {
  //     if (this.listclaimdoc[i].selected) {
  //       this.checkedList.push({
  //         p_id: this.listclaimdoc[i].id,
  //         p_paths: this.listclaimdoc[i].paths,
  //         p_file_name: this.listclaimdoc[i].file_name
  //       });
  //     }
  //   }
      ...
      if (result.value) {
        for (let J = 0; J < this.checkedList.length; J++) {
          ...
          if (this.checkedList[J].p_paths === '') {
            ...
            await this.dalservice.DeleteFile(this.dataTampPush2, this.APIController, this.APIRouteForDeleteFile)
              .toPromise().then(
                ress => {
                  const parses = JSON.parse(ress);
                  if (parses.result === 1) {
  ...
  selectAll() {
    for (let i = 0; i < this.listclaimdoc.length; i++) {
      this.listclaimdoc[i].selected = this.selectedAll;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAll = this.listclaimdoc.every(function (item: any) {
  ...
  saveList() {
    ...
    const getDate = $('[name="p_document_date"]')
      .map(function () { return $(this).val(); }).get();

    const getRemarks = $('[name="p_document_remarks"]')
      .map(function () { return $(this).val(); }).get();
        ...
        while (i < getRemarks.length) {
          if (getDate[i] !== '') {
            this.listdataDoc.push({
              p_id: getID[i],
              p_document_date: this.dateFomatList(getDate[i]),
              p_document_remarks: getRemarks[i],
            });
          }
