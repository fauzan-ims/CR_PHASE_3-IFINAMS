...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './disposeddetailassetwizlist.component.html'
})

export class DisposedDetailAssetwizlistComponent extends BaseComponent implements OnInit {
    ...
    //controller
    private APIController: String = 'DisposalDetail';
    private APIControllerHeader: String = 'Disposal';
    ...
    private RoleAccessCode = 'R00022620000001A';
    ...
    btnDeleteAll() {
        this.checkedList = [];
        for (let i = 0; i < this.listdisposaldetail.length; i++) {
            if (this.listdisposaldetail[i].selected) {
                this.checkedList.push(this.listdisposaldetail[i].id);
            }
        }
    ...
    selectAllTable() {
        for (let i = 0; i < this.listdisposaldetail.length; i++) {
            this.listdisposaldetail[i].selected = this.selectedAll;
        }
    }

    checkIfAllTableSelected() {
        this.selectedAll = this.listdisposaldetail.every(function (item: any) {
        ...
        for (let i = 0; i < this.lookupDisposalAsset.length; i++) {
            if (this.lookupDisposalAsset[i].selectedLookup) {
                ...
                this.checkedLookup.push({
                    ...
                    itemGroupCode: this.lookupDisposalAsset[i].item_group_code,
                });
            }
        }
                ...
                .subscribe(
                    res => {
                        ...
                        if (parse.result === 1) {
                            ...
                            this.dataTamp = [{
                                'p_item_group_code': this.checkedLookup[J].itemGroupCode,
                                'p_company_code': 'IMS',
                                'action': ''
                            }];
                            this.dalservice.GetrowBam(this.dataTamp, this.APIControllerMasterItem, this.APIRouteForGetRowItem)
                                .subscribe(
                                    res => {
                                        const parse = JSON.parse(res);
                                        ...
                                        // this.dataRoleTamp = [{
                                        //     'p_disposal_code': this.param,
                                        //     'p_gl_asset_code': dataItemGroup.gl_asset_code,
                                        //     'p_asset_code': this.checkedLookup[J].assetCode,
                                        //     'action': 'default'
                                        // }];
                                        ...
                                        //     this.dalservice.ExecSp(this.dataRoleTamp, this.APIController, this.APIRouteForInsertGlAssetCode)
                                        //         .subscribe(
                                        //             res => {
                                        //                 // this.showSpinner = false;
                                        //                 const parse = JSON.parse(res);
