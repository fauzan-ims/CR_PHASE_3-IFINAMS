...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './salepermitdetailassetmachinewizlist.component.html'
})

export class SellPermitAssetMachinewizdetailComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    //controller
    private APIController: String = 'AssetMachine';
    ...
    //#region  form submit
    onFormSubmit(AssetMachineForm: NgForm, isValid: boolean) {
        ...
        this.AssetMachineData = this.JSToNumberFloats(AssetMachineForm);
        const usersJson: any[] = Array.of(this.AssetMachineData);

        if (this.AssetMachineData.p_date_of_lease_agreement == null || this.AssetMachineData.p_date_of_lease_agreement === '') {
            this.AssetMachineData.p_date_of_lease_agreement = undefined;
        }
        if (this.AssetMachineData.p_start_rental_date == null || this.AssetMachineData.p_start_rental_date === '') {
            this.AssetMachineData.p_start_rental_date = undefined;
        }
        if (this.AssetMachineData.p_end_rental_date == null || this.AssetMachineData.p_end_rental_date === '') {
            this.AssetMachineData.p_end_rental_date = undefined;
        }

        this.AssetMachineData.p_total_rental_price = this.model.total_rental_price;
            ...
            .subscribe(
                res => {
                    ...
                    if (parse.result === 1) {
                        ...
                        this.route.navigate(['/transaction/assetwizlistdetail/' + this.param + '/assetdetailmachinedetail', this.param, parse.id]);
                        this.callGetrow()
                    } else {
    ...
    btnLookupModel() {
        ...
        $('#datatableLookupModel').DataTable({
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                dtParameters.paramTamp.push({
                    'p_company_code': this.company_code,
                    // 'p_type_code': this.model.type_code,
                    'p_merk_code': this.model.merk_code,
                });
